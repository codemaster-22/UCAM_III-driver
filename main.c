...
SYNC_command : AA 0D 00 00 00 00  
NAK_command : AA 0F 00 xx ee 00
xx = counter // let’s ignore as of now
ee = error code
      3)  SET_BAUDRATE_command : AA 07 31 00 00 00  ( specific for 115200 )
      4)  INITIAL_command : AA 01 00 07 07 ( specific for JPEG,640x480 )
...
xx = FF then responds immediately
      6)  SET_PACKAGE_SIZE_command : AA 06 08 00 xx 00 
                  (a)  xx = higher bits of img.size . 02 for (1<<9) size.
      7)  SNAPSHOT_command : AA 05 00 00 00 00 ( jpg format, keep current frame)
      8)  GET_PICTURE_command : AA 04 01 00 00 00 ( snapshot mode )
      9)  DATA_command : AA 0A 01 xx yy zz ( snapshot mode )
